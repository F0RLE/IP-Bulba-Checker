...
pub(crate) fn normalize_domain(input: &str) -> Option<String> {
    ...
    let value = normalized
        .strip_prefix("full:")
        .or_else(|| normalized.strip_prefix("include:"))
        .or_else(|| normalized.strip_prefix("domain:"))
        .or_else(|| normalized.strip_prefix("domain-suffix:"))
        .or_else(|| normalized.strip_prefix("DOMAIN-SUFFIX,"))
        .or_else(|| normalized.strip_prefix("DOMAIN,"))
        .or_else(|| normalized.strip_prefix("HOST,"))
        .unwrap_or_else(|| {
            if let Some(rest) = normalized
                .strip_prefix("ipset=/")
                .or_else(|| normalized.strip_prefix("server=/"))
                .or_else(|| normalized.strip_prefix("address=/"))
            {
                rest.split('/').next().unwrap_or(normalized)
            } else {
                normalized
            }
        })
    ...
    let parseable_url = if value.contains("://") {
        value
    } else {
        format!("http://{value}")
    };
...
/// Return the default per-user runtime data root for Bulbascan outputs.
pub(crate) fn runtime_data_root_dir() -> PathBuf {
    let base_dir = if cfg!(windows) {
        std::env::var_os("APPDATA").map_or_else(std::env::temp_dir, PathBuf::from)
...
pub(crate) fn default_results_dir_for_input(file: &std::path::Path) -> PathBuf {
    ...
    runtime_data_root_dir().join(format!("results_{stem}"))
}
...
pub(crate) struct Args {
    ...
    /// Try browser verification for all domains, not only profiled/ambiguous ones
    #[arg(long, default_value_t = false)]
    pub(crate) browser_all: bool,

    /// Maximum number of concurrent browser verification tabs for the shared local browser
    #[arg(long, value_name = "N")]
    pub(crate) browser_tabs: Option<usize>,
    ...
    /// Run a self-test against known blocked/direct domains to verify detection is working.
    /// Scans google.com, github.com (expect direct) and instagram.com, facebook.com, x.com,
    /// linkedin.com (expect blocked — requires --control-proxy for confirmation), deepl.com.
    /// Reports pass/fail for each case. Use with --control-proxy for full dual-vantage check.
    #[arg(long, default_value_t = false)]
...
mod tests {
    ...
    fn normalizes_domain_variants() {
        ...
        assert_eq!(
            normalize_domain("https://user:pass@example.com:8443/login"),
            Some("example.com".to_string())
        );
        assert_eq!(
            normalize_domain("HOST,example.org:8080"),
            Some("example.org".to_string())
        );
        assert_eq!(
            normalize_domain("full:.Example.com."),
            Some("example.com".to_string())
        );
        assert_eq!(
        ...
        assert_eq!(
            normalize_domain("ipset=/claude.ai/bulba_proxy"),
            Some("claude.ai".to_string())
        );
        assert_eq!(
            normalize_domain("server=/chat.openai.com/1.1.1.1"),
            Some("chat.openai.com".to_string())
        );
    ...
    fn browser_flags_parse() {
        ...
        assert!(args.browser_all);
        assert_eq!(args.browser_tabs, Some(4));
    }
    ...
    fn derives_results_dir_from_input_filename() {
        ...
        assert!(
            derived.ends_with(std::path::PathBuf::from("Bulbascan").join("results_belarus_seed"))
        );
    }
}
