...
struct ScanConfig {
    ...
    browser_all: Option<bool>,
    browser_tabs: Option<usize>,
}
...
impl FileConfig {
    ...
    fn apply(self, args: &mut Args, matches: &ArgMatches) {
        ...
        if should_apply(matches, "browser_all")
            && let Some(value) = self.scan.browser_all
        {
            args.browser_all = value;
        }
        if should_apply(matches, "browser_tabs")
            && let Some(value) = self.scan.browser_tabs
        {
            args.browser_tabs = Some(value);
        }
...
out_ok = "logs/ok.log"
out_blocked = "logs/blocked.log"
blocked_list = "lists/blocked.txt"
geosite = "bin/geosite.dat"
"#,
        )
        .expect("config");
        ...
        assert_eq!(config.output.out_ok, Some(PathBuf::from("logs/ok.log")));
        assert_eq!(
            config.output.out_blocked,
            Some(PathBuf::from("logs/blocked.log"))
        );
        assert_eq!(
            config.output.blocked_list,
            Some(PathBuf::from("lists/blocked.txt"))
        );
        assert_eq!(
            config.output.geosite,
            Some(PathBuf::from("bin/geosite.dat"))
        );
    }
}
