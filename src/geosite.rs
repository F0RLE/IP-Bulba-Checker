...
fn compile_categories(
    ...
    let ordered_categories = categories.into_iter().collect::<BTreeMap<_, _>>();

    for (cat_name, mut domains) in ordered_categories {
...
mod tests {
    ...
    #[test]
    fn geosite_categories_are_listed_in_deterministic_order() {
        let tmp = std::env::temp_dir().join(format!(
            "bulba-geosite-categories-{}.dat",
            std::process::id()
        ));
        let mut categories = HashMap::new();
        categories.insert("waf".to_string(), vec!["one.example".to_string()]);
        categories.insert("geo".to_string(), vec!["two.example".to_string()]);
        categories.insert("api".to_string(), vec!["three.example".to_string()]);

        super::compile_categories(categories, &tmp).unwrap();
        let listed = list_categories(&tmp).unwrap();
        let _ = std::fs::remove_file(&tmp);

        assert_eq!(listed, vec!["api", "geo", "waf"]);
    }
}
