...
impl LocalState {
    ...
    pub fn ingest_scan_results_conservative(&mut self, results: &[ScanResult]) {
        for result in results {
            match result.routing_decision {
                RoutingDecision::ProxyRequired => {
                    if !self.direct.contains(&result.domain) {
                        self.manual_review.insert(result.domain.clone());
                    }
                }
                RoutingDecision::DirectOk => {
    ...
    pub fn reconcile_comparison_results(&mut self, comparisons: &[ComparisonResult]) {
        for comparison in comparisons {
            match comparison.decision {
                ComparisonDecision::ConfirmedProxyRequired => {
                    self.blocked.insert(comparison.domain.clone());
                    self.direct.remove(&comparison.domain);
                    self.manual_review.remove(&comparison.domain);
                }
                ComparisonDecision::ConsistentDirect => {
                    self.blocked.remove(&comparison.domain);
                    self.direct.insert(comparison.domain.clone());
                    self.manual_review.remove(&comparison.domain);
                }
                ...
                | ComparisonDecision::ConsistentBlocked => {
                    self.blocked.remove(&comparison.domain);
                    self.direct.remove(&comparison.domain);
                    self.manual_review.insert(comparison.domain.clone());
                }
            }
        }
    }
...
mod tests {
    ...
    use crate::scanner::types::{
        ...
        ProbeEvidence, RoutingDecision, ScanResult, Verdict,
    };
    ...
    #[test]
    fn conservative_ingest_keeps_local_proxy_required_out_of_blocked_until_confirmed() {
        let mut state = LocalState::default();
        state.ingest_scan_results_conservative(&[
            sample_result("maybe-blocked.example", RoutingDecision::ProxyRequired),
            sample_result("direct.example", RoutingDecision::DirectOk),
        ]);

        assert!(!state.blocked.contains("maybe-blocked.example"));
        assert!(state.manual_review.contains("maybe-blocked.example"));
        assert!(state.direct.contains("direct.example"));
    }

    #[test]
    fn consistent_direct_comparison_removes_domain_from_blocked_state() {
        let mut state = LocalState::default();
        state.blocked.insert("flipped.example".to_string());

        state.reconcile_comparison_results(&[ComparisonResult {
            domain: "flipped.example".into(),
            service: None,
            service_role: None,
            ...
            reason: "both paths direct".into(),
        }]);

        assert!(!state.blocked.contains("flipped.example"));
        assert!(state.direct.contains("flipped.example"));
        assert!(!state.manual_review.contains("flipped.example"));
    }

    #[test]
    fn consistent_blocked_comparison_does_not_persist_as_blocked() {
        let mut state = LocalState::default();
        state.blocked.insert("ambiguous.example".to_string());

        state.reconcile_comparison_results(&[ComparisonResult {
            domain: "ambiguous.example".into(),
            service: None,
            service_role: None,
            local_verdict: Verdict::GeoBlocked,
            local_routing_decision: RoutingDecision::ProxyRequired,
            ...
            control_network_evidence: NetworkEvidence::default(),
            network_notes: Vec::new(),
            reason: "both sides blocked".into(),
        }]);

        assert!(!state.blocked.contains("ambiguous.example"));
        assert!(!state.direct.contains("ambiguous.example"));
        assert!(state.manual_review.contains("ambiguous.example"));
    }
}
