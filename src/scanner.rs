...
pub(crate) use comparison::{
    compare_with_control, summarize_service_geo_with_local_results, write_confirmed_proxy_required,
...
use transport::{build_fallback_client, build_request, send_via_http, send_with_retries};
...
fn build_scan_client(timeout_secs: u64, max_redirects: usize) -> anyhow::Result<Client> {
...
pub async fn run_scan(
    ...
    if let Some(parent) = out_ok.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await?;
    }
    if let Some(parent) = out_blocked.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await?;
    }
    ...
    // The bulk scanner now uses reqwest/rustls instead of the previous
    // HTTP/TLS path that was aborting release builds during long scans.
    let client = build_scan_client(timeout_secs, max_redirects)?;
    ...
    let local_browser_slots = if proxies.is_empty() && browser_binary.is_some() {
        Some(Arc::new(Semaphore::new(
            scan_policy.max_parallel_browser_tabs.max(1),
        )))
    } else {
        None
    };
    ...
    for worker_id in 0..absolute_max_workers {
        ...
        let local_browser_slots = local_browser_slots.clone();
...
async fn scan_domain_once(
    ...
    local_browser_slots: Option<Arc<Semaphore>>,
    ...
    if !scan_policy.browser_verify_all
        && let Some(family) = known_challenge_family
    {
        ...
        && should_try_browser_verify(&best_result, &domain, scan_policy.browser_verify_all)
        && (proxy.is_none()
            ...
            .is_ok()
    {
        ...
        {
            let probe_url = format!("https://{domain}{path}");
            let browser_dom = if proxy.is_none() {
                if let Some(browser_slots) = local_browser_slots.as_ref() {
                    let permit = browser_slots.clone().acquire_owned().await;
                    let Ok(_permit) = permit else {
                        continue;
                    };
                    run_browser_dom_dump(
                        browser_path,
                        &probe_url,
                        None,
...
async fn check_domain(
    ...
    let mut response_res =
        send_with_retries(client, &url, ua, proxy, timeout_secs, max_redirects).await;
        ...
        && !domain.starts_with("http://")
    {
        ...
        response_res = send_with_retries(
            client,
            &http_url,
            ua,
            proxy,
    ...
    match response_res {
        Ok(mut response) => {
            ...
            if should_ua_retry {
                ...
                let mut retry_req =
                    build_request(client, &url, retry_ua, proxy, timeout_secs, max_redirects)?;
        ...
        Err(e) => {
            ...
            if kind == TransportErrorKind::Unreachable {
                let fallback_client =
                    build_fallback_client(proxy.map(String::as_str), timeout_secs, max_redirects)?;
                let fallback_response = send_via_http(&fallback_client, &url, ua).await;
