    ...
    match link.security {
        ...
        SecurityKind::Reality => Some((
            ...
            json!({
                "serverName": link.server_name.clone().unwrap_or_default(),
                "fingerprint": link.fingerprint.clone().unwrap_or_else(|| "chrome".to_string()),
                // Xray client config uses "publicKey" for the server's public key (pbk in VLESS link).
                // The field was historically named "password" in some docs but Xray core requires
                // "publicKey" on the client side. Using "password" silently fails — Reality handshake
                // would reject the connection with a TLS error.
                "publicKey": link.reality_password.clone().unwrap_or_default(),
...
pub fn generate_xray_socks_client_config(
    ...
    let config = json!({
        ...
        "dns": {
            // Use only external resolvers. "localhost" would leak DNS queries through
            // the local system resolver bypassing the proxy, corrupting geo-detection.
...
mod tests {
    ...
    fn generates_reality_xhttp_client_config_from_vless_link() {
        ...
        // publicKey is the correct Xray client-side field name for the Reality server public key
        assert_eq!(
            generated.config["outbounds"][0]["streamSettings"]["realitySettings"]["publicKey"],
            "PUBLICKEY123"
        );
        assert_eq!(
        ...
        // localhost must not appear in DNS servers — it would bypass the proxy
        let dns_servers = generated.config["dns"]["servers"].as_array().unwrap();
        assert!(
            dns_servers.iter().all(|s| s != "localhost"),
            "localhost must not appear in DNS servers"
        );
    }
