...
mod tests {
    ...
    #[test]
    fn consistent_direct_comparison_removes_domain_from_review_tier() {
        let results = vec![result(
            "direct-but-noisy.example",
            RoutingDecision::ManualReview,
            Verdict::UnexpectedStatus,
        )];
        let comparisons = vec![ComparisonResult {
            domain: "direct-but-noisy.example".into(),
            service: None,
            service_role: None,
        ...
        let plan = build_publication_plan(&results, Some(&comparisons));
        assert!(plan.direct_only.contains("direct-but-noisy.example"));
        assert!(!plan.review_only.contains("direct-but-noisy.example"));
    }
}
