...
// Updated to match Chrome stable as of March 2026 (Chrome 146 released 2026-03-10).
const CURRENT_CHROMIUM_STABLE_MAJOR: u32 = 146;
const PREVIOUS_CHROMIUM_STABLE_MAJOR: u32 = CURRENT_CHROMIUM_STABLE_MAJOR - 1;
...
fn build_chromium_user_agent_pool(min_major: u32, max_major: u32) -> Vec<String> {
    let mut agents = Vec::with_capacity(32);

    for _ in 0..16 {
        let chrome_ver = fastrand::u32(min_major..=max_major);
        agents.push(format!(
...
pub fn get_random_user_agent() -> &'static str {
    let pool = UA_POOL.get_or_init(|| {
        build_chromium_user_agent_pool(
            PREVIOUS_CHROMIUM_STABLE_MAJOR,
            CURRENT_CHROMIUM_STABLE_MAJOR,
        )
    });
...
#[cfg(test)]
fn extract_chrome_major_version(ua: &str) -> Option<u32> {
    ua.split("Chrome/")
        .nth(1)
...
const SIGNATURES_BODY: &[(&str, BlockType)] = &[
    ...
    ("restricted in your", BlockType::Geo),
    ...
    // ═══ ISP / Government — RU ISP stub page domains (real redirect targets seen in the wild) ═══
    // Patterns are URL fragments that appear in redirect targets or page content.
    // Rostelecom / MGTS
    ("rostelekom-stub", BlockType::Isp),
    ("warning.rt.ru", BlockType::Isp),
    // MTS — stub redirect domain as of 2021-2024
    ("blocked.mts.ru", BlockType::Isp),
    // Beeline
    ("identity.beeline.ru", BlockType::Isp),
    // Megafon — RKN redirect pages
    ("lp.megafon.tv/rkn", BlockType::Isp),
    ("m.megafonpro.ru/rkn", BlockType::Isp),
    // ER-Telecom / Dom.ru
    ("er-telecom-block", BlockType::Isp),
    ("dom.ru/blocked", BlockType::Isp),
    // TTK
...
mod tests {
    ...
    #[test]
    fn generic_api_status_fields_do_not_count_as_block_signatures() {
        let matcher = BlockMatcher::new(None).unwrap();

        assert!(
            matcher
                .find_api_text("{\"status\":403,\"message\":\"forbidden\"}")
                .is_none()
        );
        assert!(
            matcher
                .find_api_text("{\"status\":451,\"detail\":\"legal reasons\"}")
                .is_none()
        );
    }

    #[test]
    fn ecommerce_vendor_names_do_not_count_as_geo_blocks() {
        let matcher = BlockMatcher::new(None).unwrap();

        assert!(
            matcher
                .find_body_text(
                    "<html><body>Powered by Borderfree for global checkout</body></html>"
                )
                .is_none()
        );
        assert!(
            matcher
                .find_body_text("<html><body>Global ecommerce experience by ESW.com</body></html>")
                .is_none()
        );
    }

    #[test]
    fn generic_http_error_words_do_not_count_as_waf_or_geo_by_themselves() {
        let matcher = BlockMatcher::new(None).unwrap();

        assert!(
            matcher
                .find_body_text("<html><body><h1>403 Forbidden</h1></body></html>")
                .is_none()
        );
        assert!(
            matcher
                .find_body_text(
                    "<html><body>We currently provide limited service in beta.</body></html>"
                )
                .is_none()
        );
        assert!(
            matcher
                .find_body_text(
                    "<html><body>This feature is not currently supported on mobile.</body></html>"
                )
                .is_none()
        );
    }
    ...
    #[test]
    fn generic_regulator_mentions_do_not_count_as_block_pages() {
        let matcher = BlockMatcher::new(None).unwrap();

        assert!(
            matcher
                .find_body_text(
                    "<html><body>News article about Roskomnadzor policy changes.</body></html>"
                )
                .is_none()
        );
        assert!(
            matcher
                .find_body_text(
                    "<html><body>Новость о Роскомнадзоре и новых правилах.</body></html>"
                )
                .is_none()
        );
    }
    ...
    #[test]
    fn user_agent_pool_tracks_current_chromium_stable_window() {
        for _ in 0..100 {
            let ua = get_random_user_agent();
            let version =
                extract_chrome_major_version(ua).expect("ua should contain a Chrome major version");

            assert!(
                (PREVIOUS_CHROMIUM_STABLE_MAJOR..=CURRENT_CHROMIUM_STABLE_MAJOR).contains(&version)
            );
        }
    }
}
