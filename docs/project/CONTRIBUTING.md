...
- Check the current repository status first. Bulbascan is in a temporary maintenance pause, so only small, clearly scoped fixes are likely to make sense.
...
| Area | Why it matters |
|---|---|
| **Detection accuracy** | WAF vs GeoBlock confusion still causes real false positives in router configs |
...
```sh
docker build -f docker/Dockerfile.dev -t bulbascan-dev .
...
docker compose -f docker/docker-compose.dev.yml run --rm bulbascan-dev cargo check
docker compose -f docker/docker-compose.dev.yml run --rm bulbascan-dev cargo test
docker compose -f docker/docker-compose.dev.yml run --rm bulbascan-dev cargo clippy -- -D warnings
```
